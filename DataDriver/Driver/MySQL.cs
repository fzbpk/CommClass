...
namespace NK.Data
{
    public partial class MySql : IDisposable, iDataBase
    {
        ...
        public DbConnection GetConnection()
        {
            try
            {
                DbConnection conn = new MySqlConnection(this.Connection);
                conn.Open();
                return conn;
            }
            catch (Exception ex)
        ...
        public bool TableIsExist(string TableName)
        {
            ...
            try
            {
                bool res = false;
                MySqlConnection conn = (MySqlConnection)GetConnection();
                MySqlCommand cmd = new MySqlCommand("select count( * ) FROM information_schema.TABLES WHERE table_name = '" + TableName + "' AND TABLE_SCHEMA = '" + conn.Database + "'", conn);
                cmd.CommandTimeout = this.Timeout * 1000;
