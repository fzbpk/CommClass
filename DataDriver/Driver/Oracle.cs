...
namespace NK.Data
{
    public partial class Oracle : IDisposable, iDataBase
    {
        ...
        public DbConnection GetConnection()
        {
            try
            {
                DbConnection conn = new OracleConnection(this.Connection);
                conn.Open();
                return conn;
            }
            catch (Exception ex)
        ...
        { 
            ...
            try
            {
                bool res = false;
                OracleConnection conn = (OracleConnection)GetConnection();
                OracleCommand cmd = new OracleCommand("SELECT COUNT(*) FROM USER_OBJECTS WHERE UPPER(OBJECT_NAME)='" + TableName.ToUpper() + "'", conn);
                cmd.CommandTimeout = this.Timeout * 1000;
        ...
        public DataTable getDataTableByRam(int PageIndex, int PageSize, string DisplayField, string TableName, string Where, string OrderBy, string GroupBy, out int RecodeCount, out int PageCount)
        {
            ...
            try
            {
                ...
                OracleConnection conn = (OracleConnection)GetConnection();
                OracleCommand cmd = new OracleCommand(CountSql, conn);
                cmd.CommandTimeout = this.Timeout * 1000;
        ...
        public bool CheckField(string TableName, string Field, out System.Type FieldType, out bool CanBeNull, out bool IsPrimaryKey)
        {
            ...
            try
            {
                ...
                if (res)
                {
                    OracleCommand cmd = new OracleCommand("select " + Field + " from " + TableName + "", conn);
                    cmd.CommandTimeout = this.Timeout * 1000;
                    OracleDataReader da = cmd.ExecuteReader();
