...
namespace NK.Data
{
    public partial class PostgreSQL : IDisposable, iDataBase
    {
        ...
        public DbConnection GetConnection()
        {
            try
            {
                DbConnection conn = new NpgsqlConnection(this.Connection);
                conn.Open();
                return conn;
            }
            catch (Exception ex)
        ...
        { 
            ...
            try
            {
                bool res = false;
                NpgsqlConnection conn = (NpgsqlConnection)GetConnection();
                NpgsqlCommand cmd = new NpgsqlCommand("select count(1) from  TableName ", conn);
                cmd.CommandTimeout = this.Timeout * 1000;
        ...
        public DataTable getDataTableByRam(int PageIndex, int PageSize, string DisplayField, string TableName, string Where, string OrderBy, string GroupBy, out int RecodeCount, out int PageCount)
        {
            ...
            try
            {
                ...
                NpgsqlConnection conn = (NpgsqlConnection)GetConnection();
                NpgsqlCommand cmd = new NpgsqlCommand(CountSql, conn);
                cmd.CommandTimeout = this.Timeout * 1000;
   ...
   + " where temprownumber>" + ((PageIndex - 1) * PageSize).ToString();
        ...
        public bool CheckField(string TableName, string Field, out System.Type FieldType, out bool CanBeNull, out bool IsPrimaryKey)
        {
            ...
            try
            {
                ...
                if (res)
                {
                    NpgsqlCommand cmd = new NpgsqlCommand("select " + Field + " from " + TableName + "", conn);
                    cmd.CommandTimeout = this.Timeout * 1000;
                    NpgsqlDataReader da = cmd.ExecuteReader();
