...
namespace NK.Data
{
    public partial class SQLite : IDisposable, iDataBase
    {
        ...
        public DbConnection GetConnection()
        {
            try
            {
                DbConnection conn = new SQLiteConnection(this.Connection);
                conn.Open();
                return conn;
            }
            catch (Exception ex)
        ...
        { 
            ...
            try
            {
                bool res = false;
                SQLiteConnection conn = (SQLiteConnection)GetConnection();
                SQLiteCommand cmd = new SQLiteCommand("select count(1) from " + TableName + "", conn);
                cmd.CommandTimeout = this.Timeout * 1000;
                CloseConnection(conn);
                return res;
            }
        ...
        public DataTable getDataTableByRam(int PageIndex, int PageSize, string DisplayField, string TableName, string Where, string OrderBy, string GroupBy, out int RecodeCount, out int PageCount)
        {
            ...
            try
            {
                ...
                SQLiteConnection conn = (SQLiteConnection)GetConnection();
                SQLiteCommand cmd = new SQLiteCommand(CountSql, conn);
                cmd.CommandTimeout = this.Timeout * 1000;
